const Engine = Matter.Engine;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Body = Matter.Body;

var ball,groundObj,leftSide,rightSide;
var dustbinImg,paperImg;
//var dustbin,paper;
var world;
var radius = 70;

function preload(){
	dustbinImg = loadImage("dustbin.png");
	paperImg = loadImage("paper.png");

	
}


function setup() {
	
	

	createCanvas(1600, 700);
	rectMode(CENTER);

	engine = Engine.create();
	world = engine.world;

	//dustbin = createSprite(200,180,400,20);
	//dustbin.x = dustbin.width /2;
	//dustbinImg = addImage("dustbin.png");
	
	
	//paper = createSprite(200,180,400,20);
  //	paper.x = paper.width /2;
//	paperImg = addImage("paper.png");
	

	var ball_options={
		isStatic:false,
		restitution:0.3,
		density:0.4
	}

	ball = Bodies.circle(260,100,radius/2.6,ball_options);
	World.add(world,ball);

	ground=new Ground(width/2,670,width,20);
	leftSide = new Ground(1100,600,10,120);
	rightSide = new Ground(1270,600,10,120);
	bottomSide = new Ground(1185,650,150,20);

	dustbinImg = loadImage("dustbin.png");
	paperImg = loadImage("paper.png");

	Engine.run(engine);
  
}


function draw() {
	background(200);
	rectMode(CENTER);

	ground.display();
	leftSide.display();  
	rightSide.display();
	bottomSide.display();

	
	imageMode(CENTER);

//image(paper.position.x,ball.position.y,radius,radius);


	image(dustbinImg,1185, 560, 200,200);
	//image(dustbin.position.x,ball.position.y,radius,radius);
	image(paperImg,200, 630, 80,80);
	//paper.scale=0.01;
}

function keyPressed() {
  	if (keyCode === UP_ARROW) {

		Matter.Body.applyForce(paperImg,paperImg.position,{x:85,y:-85});
    
  	}
}
